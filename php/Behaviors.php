...
function attivitaFisica($resp,$parameters,$text,$email){
    ...
    if(isset($parameters['date-period']['startDate'])){
        ...
        if($arr[0] == 0 && $arr[1] == 0 && $arr[2] == 0){
            ...
            if($activity[0] == 0 && $activity[1] == 0 && $activity[2] ==  0)
                return "Sembra che tu non abbia svolto attivitÃ  fisica ðŸ˜…";


            $answer = "Gli ultimi dati disponibili sono del ".$activity[3].", quando hai svolto  ";
            $answer .=  $activity[2] ." minuti di attivitÃ  molto attiva ".
                $activity[1] ." minuti di attivitÃ  poco attiva e ";

            $answer .=  $activity[0]." minuti di attivitÃ  abbastanza attiva<br><br>";

        }else{

            //risposta con intervallo
            $answer = "In media svolgi ";
            $answer .= $arr[0]." minuti di attivitÃ  molto attiva, ";
            $answer .= $arr[1]." minuti di attivitÃ  poco attiva <br>";
            $answer .= $arr[2]." minuti di attivitÃ  abbastanza attiva <br>";
        ...
        if($date  == $activity[3]){
            ...
            $answer .=  $activity[2] ."minuti di attivitÃ  molto attiva <br> ".
                $activity[1] ." minuti di attivitÃ  poco attiva e ";
...
function attivitaFisicaBinary($resp,$parameters,$text,$email){
    ...
    if(isset($parameters['date-period']['startDate'])){
        ...
        if(strpos($text, 'abbastanza')){
            ...
            if($sum >= 30 ){
                $answer ="Si, fai abbastanza attivitÃ  fisica. In media ".$sum." minuti di attivitÃ .";
            }else{
                $answer="No, non fai abbastanza attivitÃ  fisica. In media ".$sum." minuti di attivitÃ .";
            }

        }elseif(strpos($text,'dovrei fare')  || strpos($text,'fare di piÃ¹') || strpos($text,'fare piÃ¹')) {

            if($sum >= 30 ){
                $answer ="No, fai abbastanza attivitÃ  fisica. In media ".$sum." minuti di attivitÃ .";
            }else{
                $answer="Si, non fai abbastanza attivitÃ  fisica. In media ".$sum." minuti di attivitÃ .";
            }
        ...
        if(strpos($text, 'abbastanza')){
            ...
            if($sum >= 30 ){
                $answer .=" hai fatto abbastanza attivitÃ  fisica. Ovvero ".$sum." minuti di attivitÃ .";
            }else{
                $answer .=" non hai fatto abbastanza attivitÃ  fisica. Ovvero ".$sum." minuti di attivitÃ .";
            }
        ...
        $answer = "Sfortunatamente non sono riuscito a trovare dati sulla tua attivitÃ  fisica ðŸ˜…";
    }
    ...
    if(isset($parameters['date-period']['startDate'])){
        ...
        if(strpos($text, 'abbastanza')){
            ...
            if($calAv >= $metabolismo)
                ...
                $answer = "No, dovresti bruciare piÃ¹ calorie";

        }elseif (strpos($text, 'piÃ¹')) {

            if($calAv >= $metabolismo)
                $answer = "No, bruci abbastanza calorie";
            else
                $answer = "Si,dovresti bruciare piÃ¹ calorie";
    ...
    if(isset($parameters['date-period']['startDate'])){
        ...
        if($intAv[0] == $startDate){
            ...
            if($intAv[1] >= $average[1]){
                $answer = "Si, hai fatto abbastanza passi. La tua media giornaliera Ã¨ di ".$intAv[1]." passi";
            }else{
                $answer = "No, non hai fatto abbastanza passi. La tua media giornaliera Ã¨ di ".$intAv[1]." passi";
            }
...
function getSedentaryBinary($resp,$parameters,$text,$email){
    ...
    if($result == null){
        return "Sfortunatamente non sono stati trovati dati sufficienti per rispondere ðŸ˜…";
    }
...
function getDistance($resp, $parameters, $text, $email)
{
    ...
    if ($distance == 0.00) {
        $answer = "Dai miei dati non risultano distanze percorse";
    } else {
